...
public class _06_Task_3 extends GenelWebDriver {
    ...
    void specialsValidation()
    {
        ...
        List<WebElement> oldPriceList = driver.findElements(By.className("price-old")); // eski fiyatlı urun listesi
        List<WebElement> productList = driver.findElements(By.className("product-thumb")); // urun isimleri h4>a
        ...
        for (int i = 0; i < newPriceList.size(); i++) {
            Assert.assertTrue(Tools.WebElementToDouble(newPriceList.get(i))<Tools.WebElementToDouble(oldPriceList.get(i)));
        }
    }
